...
MODEL_PATH = os.path.join(BASE_DIR, "models", "Feedback_KD", "corr_kd_model_2023.pt")
LOG_PATH   = os.path.join(BASE_DIR, "logs",   "Feedback_KD", "corr_kd_history_2023.json")
...
def corr_kd_loss(student_embed: torch.Tensor,
    ...
    s_aligned = align(student_embed)
    Es = _center(s_aligned)
    Et = _center(teacher_embed)
...
def train_corr_kd_2023(seed: int = 1337, epochs: int = 20, alpha: float = 0.7, batch_size: int = 512):
    set_seed(seed)
    print("🚀 Training CorrKD Student on CICIoT2023...")
    ...
    print(f"✅ Device: {device} | Batch size: {batch_size} | alpha: {alpha}")
